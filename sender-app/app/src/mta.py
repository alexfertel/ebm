...
class Broker:
    ...
    def process_data(self):
        ...
        if len(self.messages[subject['message_id']]) == int(subject['message_id'].split('C')[1].split('B')[0]):
    ...
    def merge(items: list) -> tuple:
        ...
        with open(f'{os.path.join(RECEIVED_FOLDER,items[0].subject["name"])}', "w+") as f:
            for block in items:
